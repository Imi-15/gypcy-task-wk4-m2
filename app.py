...
if st.button("🚀 Extract Features", use_container_width=True):
    if image is None and image_input_type == "Upload":
        ...
        if image_features is not None:
            ...
            with tab3:
                if text_embedding is not None:
                    st.header("Image ↔ Text Comparison")
                    st.write("Image features and text embeddings are shown separately because their dimensions differ.")
                    st.write("- Image features: 512-dimensional vector from ResNet18")
                    st.write("- Text embedding: 384-dimensional vector from all-MiniLM-L6-v2")
                    st.write("If you want to compare them directly, use models that produce embeddings in the same dimension.")
